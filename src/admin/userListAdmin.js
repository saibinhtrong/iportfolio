...
const userListAdmin = ()=>{
    ...
    useEffect(()=>{
        axios.get("http://localhost:3000/APIproject").then(({data})=>setUser(data))
    },[])
    useEffect(()=>{
    ...
    <h1 class="text-center text-[#4018d1] font-bold">MY USER</h1>
            ...
            <tbody>
                <tr class="text-center">
                    ...
                    <td class="border text-[#ffff]">
                        <button data-id=${item.id} class="bg-blue-600 p-5 py-2 text-[#ffff] btn_delete" id="btn_delete">DELETE</button>
                    </td>
                </tr>
        </tbody>
            `
